...
## Архитектура
Класс UNet реализует архитектуру нейросети U-Net.

**Методы:**
  encode(...): сжатие входного изображения через энкодер с сохранением промежуточных признаков.
    
  decode(...): восстановление изображения через декодер с применением пропускающих соединений.
  
  forward(...): полное прохождение сигнала через сеть (прямой проход).
  
  dice_loss(...): вычисляет потерю по метрикам Dice Loss.

  
**Вспомогательные классы:**
  ConvLayer: примитивный свёрточный слой.
    
  PoolLayer: примитивный слой пулинга.
  
  UpConvLayer: примитивный слой трансконволюции (upsampling).
